...
def wordstatsindex():
    if request.method == 'POST':
        ...
        total_words = wordstats.total_words(user_string)
        total_chars = wordstats.total_chars(user_string)
        word_initials = wordstats.initials(user_string)
        return render_template('wordstats.html', average_length=average_length, total_words=total_words, total_chars=total_chars, word_initials=word_initials)
    else:
